...
import { ScrollView, TextInput, Modal, Pressable, KeyboardAvoidingView, Platform } from 'react-native';
import { Ionicons } from '@expo/vector-icons';
import { useCommissioningTasksScreenController } from '@/controllers/commissioningTasksController';
import { StatCard } from '@/_components/commissioningTasks/_components/StatCard';
import { SrJobCard } from '@/_components/commissioningTasks/_components/SrJobCard';
import { formatTaskType, formatAddress } from '@/_components/commissioningTasks/helpers';
...
export default function CommissioningTasksScreen() {
  const {
    ...
    getFilteredSrTasks,
    handleLogout,
  } = useCommissioningTasksScreenController();
...
>
        <View>
          ...
          <View style={styles.statsContainer}>
            <StatCard title="Total"     value={srDashboard.total}     color="#241D67" />
            <StatCard title="Active"    value={srDashboard.active}    color="#241D67" />
            <StatCard title="Completed" value={srDashboard.completed} color="#241D67" />
            <StatCard title="Closed"    value={srDashboard.closed}    color="#241D67" />
          </View>
        </View>
...
getFilteredSrTasks().map(task => {
  // NEW
const effectiveStatus = srTaskStatusOverrides[task._id] || task.status;
return (
  <SrJobCard
    key={task._id}
    task={task}
    effectiveStatus={effectiveStatus}
    isLoading={!!srTaskActionLoading[task._id]}
    errorMsg={srTaskActionError[task._id]}
    onAccept={() => handleAcceptSrTask(task._id)}
    onStart={() => handleStartSrTask(task._id)}
