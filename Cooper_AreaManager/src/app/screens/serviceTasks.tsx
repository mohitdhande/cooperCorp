...
import { View, TouchableOpacity, StyleSheet, FlatList, RefreshControl, useWindowDimensions } from 'react-native';
...
export default function ServiceTasksScreen() {
  ...
  // Unified list data — same reasoning as commissioningTasks.tsx's own
  // listData: the search-results branch and the normal task-list branch
  // rendered functionally identical TaskPreviewCard blocks, just from two
  // differently-shaped sources. Flattened here so renderTaskCard below only
  // needs to exist once, and so this screen rides a single FlatList
  // instead of a ScrollView + .map() that used to mount every task at once
  ...
  const renderEmptyState = () => {
    ...
    return <Text style={styles.statusText}>No {selectedTab.toLowerCase()} service tasks.</Text>;
  };
  ...
  // every task in the list at once. Shared by both the search-results and
  // normal task-list cases (listData above already flattens either source
  // to the same plain task[] shape).
  const renderTaskCard = ({ item: task }: { item: (typeof tasks)[number] }) => {
    // Active tab normally has no arrow/start-continue button — a dealer
    // still gets Accept (ASSIGNED) and the assign icon there, since Active
    // is the only tab where those statuses ever appear for them. An area
    // manager who assigned this task to themselves (or had it self-
    // assigned/handed to them) is in the exact same position a dealer is —
    // they need to be able to Accept/Start it right here too, not just
    // from the Dashboard's own Active Task carousel. Any role can act on a
    // task genuinely assigned to them (most commonly an engineer) — not
    // just the dealer/AM self-assigned cases this used to be scoped to.
    // Without this, an engineer's own task (at ANY Active-tab status,
    // including COMPLETED/OTP-pending) had no arrow here at all, and
    // COMPLETED specifically doesn't even appear on the Dashboard's own
    ...
    return (
      ...
      // contentContainerStyle) — the header sections below already carry
      ...
      <TaskPreviewCard
        ...
        // accepting on behalf of an engineer isn't a real action (the
        // backend rejects it: "not assigned to you"). Doesn't affect an
        // engineer's own Accept (canActInActiveTab already covers that via
        // isMyOwnTask on its own).
        onAcceptPress={selectedTab === 'Active' && (!canActInActiveTab || (isDealer && !isMyOwnTask)) ? undefined : () => handleAcceptTask(task._id)}
        // A dealer's self-assigned task follows the same Accept/Start/
        // Complete flow as an engineer's own task (canActInActiveTab above
        // already grants the arrow) — Assign/Reassign only makes sense for
        // a task actually handed off to one of their engineers.
